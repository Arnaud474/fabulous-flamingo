...
public class Board {
	...
	public void updateBoard(String move) {
		...
		// System.out.println("Player " + (piece.getColor() == 4 ? "White" :
		// "Black") + " moved from " + moves[0] + " to " + moves[1]);
		// System.out.println("---------------------------------");
	...
	private void recountQuads() {
		...
		for (int i = 0; i < 9; i++) {
			for (int j = 0; j < 9; j++) {
				quad[2][i][j] = quadValue(i, j, 2); // # of pieces in quad, 5
													// for diagonals per side
	 ...
	 *
	 * @param x
	 *            integer value for quad x
	 * @param y
	 *            integer value for quad y
	 * @param side
	 *            integer of player to calculate (PLAYER_WHITE, PLAYER_BLACK)
	 ...
	 *
	 * @param player
	 *            int The player to test [PLAYER_WHITE,PLAYER_BLACK]
	 * @param x
	 *            int The column value to test.
	 * @param y
	 *            int The row value to test.
